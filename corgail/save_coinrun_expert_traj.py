...
#
#
#     traj_actions = np.zeros((n_episodes,env.action_space.n), dtype=float)
#     traj_observations = np.zeros(((n_episodes,*obs_space)), dtype=float)
#     traj_rewards = np.zeros((n_episodes,     num_steps_per_ecoch = 100
# ), dtype=float)
#     traj_lens = np.zeros(traj_lens)

    while ep_idx in range(n_episodes):
        if record_images:
            image_path = os.path.join(image_folder, "{}-{}.{}".format(ep_idx,idx, image_ext))
            obs_ = obs[0]
        ...
        one_hot_action = [0] * env.action_space.n
        one_hot_action[action[0].item()] = 1
    ...
    if save_images: # if we are saving the image locations instead of the content of the images.
    ...
    # torch.save(data, save_path, pickle_protocol=pickle.HIGHEST_PROTOCOL)
    # np.save(save_path, data)
    # with open(data, 'wb') as f:
    # np.savez(save_path, actions = data['actions'], states = data['states'], rewards = data['rewards'],
    #          lengths = data['lengths'])

    # To get around some picke issues for large files
...
if __name__=='__main__':
    load_dir = 'trained_models/coinrun-best-500level'
    env_name = 'CoinRun'
    # load_dir = 'trained_models/ppo/'
    # env_name = 'PongNoFrameskip-v4'

    # load_dir = 'trained_models/gpu/Random-Mazes-40M-Level1000/'
    # env_name = 'Random-Mazes'
    generate_expert_traj(load_dir=load_dir, save_path='gail_experts/trajs_{}.pt'.format(env_name.lower()),
                         env_name=env_name, n_episodes=100, save_images=False, image_folder='recorded_images', seed=0)
